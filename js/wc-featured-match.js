(function () {
  ...
  const RESULTS = [
    { match: 'France vs England', score: '4-6', note: 'England secured third place in a ten-goal bronze final' },
  ...
  function splitTimes(times) { return String(times || '').split(' - ').map(function (part) { const pieces = part.split(' '); return { label: pieces[0] || '', time: pieces.slice(1).join(' ') || '' }; }); }

  function renderFeaturedCard() {
    const match = MATCHES[0];
    setText('wcConfidence', 'AI Confidence ' + match.confidence + '%');
    setText('wcHomeTeam', match.home); setText('wcAwayTeam', match.away);
    setText('wcHomeNote', match.stage); setText('wcAwayNote', match.stage);
    setText('wcVersus', 'VS'); setText('wcAdvice', match.advice);
    setText('wcHomePercent', match.homePercent + '%'); setText('wcDrawPercent', match.drawPercent + '%'); setText('wcAwayPercent', match.awayPercent + '%');
    setText('wcMatchDate', match.date + ', ' + match.time); setText('wcMatchStatus', match.tag); setText('wcVenue', match.venue);
  ...
  function renderHero() {
    ...
    if (pill) pill.textContent = 'World Cup Final Day';
    if (title) title.innerHTML = 'Choose your colours. <span>Back your final team.</span>';
    if (intro) intro.textContent = 'Spain and Argentina play for the trophy after England secured bronze in a spectacular 6-4 win over France. Pick your side and support your team until the final whistle.';
  ...
  function renderPredictions() {
    ...
    if (title) title.textContent = 'World Cup Final AI Prediction';
    if (!box) return;
    box.className = 'bf-predictions-table bf-clean-predictions';
    box.innerHTML = '<div class="bf-clean-grid">' + MATCHES.map(function (match) {
      const times = splitTimes(match.localTimes).map(function (item) { return '<span>' + item.label + '<b>' + item.time + '</b></span>'; }).join('');
  ...
  function renderLowerPanels() {
    ...
    view.innerHTML = '<article class="bf-line-panel"><div class="bf-line-head"><h2>Next Match</h2><span>Support your team</span></div><div class="bf-line-list">' + MATCHES.map(function (match) {
    ...
    }).join('') + '</div></article><article class="bf-line-panel"><div class="bf-line-head"><h2>Latest Results</h2><span>Final weekend</span></div><div class="bf-line-list">' + RESULTS.map(function (result) {
  ...
  function renderSearch() {
    ...
    if (tags) tags.innerHTML = MATCHES.map(function (match) { return '<button type="button">' + match.home + ' vs ' + match.away + '</button>'; }).join('');
    if (!input || !button || !result) return;
    function analyze(value) {
      const query = cleanText(value).toLowerCase();
      const match = MATCHES.find(function (item) { return (item.home + ' vs ' + item.away).toLowerCase() === query; }) || MATCHES[0];
