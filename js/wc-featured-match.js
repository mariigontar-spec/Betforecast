(function () {
  const MATCHES = [
    ...
    },
    {
      ...
      pick: 'Argentina DNB',
      tag: 'SF',
      advice: 'Argentina hold a narrow technical edge, with England dangerous in transition',
      text: 'Argentina project slightly stronger ball control. England remain a serious threat through pace, set pieces and direct attacks.',
  ...
  const RESULTS = [
    {
      ...
      score: '3-1',
      note: 'Argentina advanced to the semifinal'
    },
    {
      match: 'Norway vs England',
      score: '1-2',
      note: 'England advanced to the semifinal'
    },
    {
  ...
  function renderPredictions() {
    ...
    box.innerHTML = `<div class="bf-clean-grid">${MATCHES.map(function (m) {
      return `<article class="bf-clean-match-card"><div class="bf-clean-card-top"><strong class="bf-clean-match-title">${m.home} vs ${m.away}</strong><span class="bf-clean-stage">${m.stage}</span></div><div class="bf-clean-probs"><div><span>Home</span><b>${m.homePercent}%</b></div><div><span>Draw</span><b>${m.drawPercent}%</b></div><div><span>Away</span><b>${m.awayPercent}%</b></div></div><span class="bf-clean-pill">${m.pick}</span><small class="bf-clean-meta">${m.date}, ${m.time} - ${m.venue}</small></article>`;
    }).join('')}</div>`;
  }
  ...
  function renderLowerPanels() {
    ...
    modelView.innerHTML = `<article class="bf-line-panel"><div class="bf-line-head"><h2>Upcoming Matches</h2><span>Fixtures</span></div><div class="bf-line-list">${MATCHES.map(function (m) {
  ...
  function renderSearch() {
    ...
    if (input) input.placeholder = 'e.g. France vs Spain';
    if (tags) tags.innerHTML = MATCHES.map(function (m) { return `<button type="button">${m.home} vs ${m.away}</button>`; }).join('');
    if (!input || !button || !result) return;
    const data = {};
    MATCHES.forEach(function (m) { data[(m.home + ' vs ' + m.away).toLowerCase()] = m; });
    function analyze(value) {
      ...
      result.innerHTML = `<div class="bf-live-result"><h3>${m.home} vs ${m.away}</h3><p><strong>Home:</strong> ${m.homePercent}% - <strong>Draw:</strong> ${m.drawPercent}% - <strong>Away:</strong> ${m.awayPercent}%<br><strong>AI pick:</strong> ${m.pick}<br><strong>Confidence:</strong> ${m.confidence}%<br>${m.text}</p></div>`;
    }
    ...
    document.querySelectorAll('.bf-tags button').forEach(function (tag) {
      tag.onclick = function () { input.value = tag.textContent.trim(); analyze(input.value); };
    });
  }
